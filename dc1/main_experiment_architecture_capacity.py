...
ARCHITECTURE_NAMES = {
    ...
    "stronger_custom_cnn": "Stronger Custom CNN",
}
...
class BalancedBatchIterator:
    ...
    This avoids changing the rest of the repository while still giving this test a
    stable batching path. In balanced mode, each epoch downsamples every class to the
    minority-class count, then shuffles the pooled indices before batching.
    """
...
class StrongerCustomCNN(nn.Module):
    """
    Test-1 architecture:
    - 4 conv blocks
    - channels: 32 -> 64 -> 128 -> 256
    - adaptive average pooling
    - classifier head: 256 -> 128 -> n_classes

    Trained from scratch to test whether better custom capacity/design materially
    improves performance over the current baseline Net.
    """
...
def build_model(architecture_name: str, n_classes: int) -> nn.Module:
    if architecture_name == "baseline_net":
        return Net(n_classes=n_classes)
    if architecture_name == "stronger_custom_cnn":
        return StrongerCustomCNN(n_classes=n_classes)
...
def train_single_architecture(
    ...
    display_name = ARCHITECTURE_NAMES[architecture_name]
    experiment_name = f"{args.experiment_name_prefix}_{architecture_name}"
    experiment_root = base_dir / "experiments" / args.experiment_group / architecture_name
    ...
    print("\n" + "=" * 90)
    print(f"Starting architecture run: {display_name}")
    ...
    if best_state_dict is None or best_epoch is None or best_confusion_matrix is None:
        raise RuntimeError(f"No valid best checkpoint was created for '{architecture_name}'.")
    ...
    run_config: Dict[str, object] = {
        ...
        "reference_balanced_batch_macro_f1": args.reference_macro_f1,
        "reference_threshold_macro_f1": args.reference_threshold_macro_f1,
        "beats_reference_balanced_batch": bool(best_val_macro_f1 > args.reference_macro_f1),
    ...
    final_report: Dict[str, object] = {
        ...
        "final_validation_loss": history["validation_loss"][final_epoch_idx],
        "final_validation_accuracy": history["validation_accuracy"][final_epoch_idx],
        "final_validation_macro_f1": history["validation_macro_f1"][final_epoch_idx],
        "final_model_path": str(final_model_path) if final_model_path is not None else None,
    }
    ...
    summary_lines = [
        f"Architecture experiment finished: {display_name}",
        ...
        f"Reference balanced-batch macro-F1: {args.reference_macro_f1:.6f}",
        f"Reference thresholded macro-F1: {args.reference_threshold_macro_f1:.6f}",
        f"Beats balanced-batch reference: {best_val_macro_f1 > args.reference_macro_f1}",
    ...
    print(f"\nCompleted architecture run: {display_name}")
...
def parse_architecture_list(raw_items: Sequence[str]) -> List[str]:
    allowed = {"baseline_net", "stronger_custom_cnn"}
    parsed: List[str] = []
    ...
    for item in raw_items:
        for token in item.split(","):
            ...
            if name not in allowed:
                raise ValueError(f"Unsupported architecture '{name}'. Choose from: {sorted(allowed)}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Architecture capacity experiment for Test 1: compare the current baseline Net "
            "against a stronger custom CNN, both trained from scratch under the same split and setup."
        )
    )
    parser.add_argument("--experiment_group", type=str, default="experiment_architecture_capacity")
    parser.add_argument("--experiment_name_prefix", type=str, default="architecture_test1")
    parser.add_argument(
        "--architectures",
        nargs="+",
        default=["baseline_net", "stronger_custom_cnn"],
        help="Architectures to run. Choices: baseline_net, stronger_custom_cnn",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Use balanced batches during training. Recommended default: True.",
    )
    ...
    parser.add_argument(
        "--reference_macro_f1",
        type=float,
        default=0.26966423915511967,
        help="Current balanced-batch validation macro-F1 reference to beat.",
    )
    parser.add_argument(
        "--reference_threshold_macro_f1",
        type=float,
        default=0.2803132184757912,
        help="Current thresholded balanced-batch validation macro-F1 reference to beat.",
    )
    return parser
...
def main() -> None:
    ...
    architecture_names = parse_architecture_list(args.architectures)
    ...
    lines = [
        "Architecture capacity comparison finished.",
        f"Run group ID: {run_group_id}",
        f"Experiment group: {args.experiment_group}",
        f"Architectures: {', '.join(str(r['architecture_display_name']) for r in results)}",
        f"Reference balanced-batch macro-F1: {args.reference_macro_f1:.6f}",
        f"Reference thresholded macro-F1: {args.reference_threshold_macro_f1:.6f}",
        "",
    ...
    for result in results:
        lines.extend(
            [
                f"- {result['architecture_display_name']}:",
                ...
                f"  beats_reference_balanced_batch = {result['beats_reference_balanced_batch']}",
    ...
    print("\n@@@ All architecture runs finished.")
