...
ARCHITECTURE_NAMES = {
    "frozen_resnet18": "Frozen ResNet18",
    "finetuned_resnet18": "Fine-tuned ResNet18",
}
...
def build_model(architecture_name: str, n_classes: int) -> ResNet18Transfer:
    if architecture_name not in {"frozen_resnet18", "finetuned_resnet18"}:
        raise ValueError(f"Unsupported architecture_name='{architecture_name}'")
    return ResNet18Transfer(n_classes=n_classes, mode=architecture_name)
...
def train_single_architecture(
    ...
    print("\n" + "=" * 94)
    print(f"Starting transfer-learning run: {display_name}")
    print(f"Experiment name        : {experiment_name}")
    print(f"Balanced train batches : {args.balanced_batches}")
    print(f"Head learning rate     : {args.head_lr}")
    print(f"Backbone learning rate : {args.backbone_lr if architecture_name == 'finetuned_resnet18' else 'frozen'}")
    ...
    run_config: Dict[str, object] = {
        ...
        "backbone_learning_rate": args.backbone_lr if architecture_name == "finetuned_resnet18" else None,
        "weight_decay": args.weight_decay,
    ...
    summary_lines = [
        f"Transfer-learning experiment finished: {display_name}",
    ...
    print(f"\nCompleted transfer-learning run: {display_name}")
...
def parse_architecture_list(raw_items: Sequence[str]) -> List[str]:
    allowed = {"frozen_resnet18", "finetuned_resnet18"}
    parsed: List[str] = []
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Transfer-learning experiment for Test 2: compare frozen and fine-tuned pretrained "
            "ResNet18 variants under the same split and setup."
        )
    )
    parser.add_argument("--experiment_group", type=str, default="experiment_resnet18_transfer")
    parser.add_argument("--experiment_name_prefix", type=str, default="architecture_test2")
    parser.add_argument(
        "--architectures",
        nargs="+",
        default=["frozen_resnet18", "finetuned_resnet18"],
        help="Architectures to run. Choices: frozen_resnet18, finetuned_resnet18",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable deterministic training. Use this if you still encounter CUDA deterministic "
            "issues or want maximum speed."
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    lines = [
        "ResNet18 transfer-learning comparison finished.",
    ...
    print("\n@@@ All transfer-learning runs finished.")
