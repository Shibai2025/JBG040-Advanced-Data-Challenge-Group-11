...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run Grad-CAM analysis on the test set and export reports."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a specific checkpoint file. If not provided, the newest checkpoint will be selected automatically."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory containing checkpoint files. Default: <script_dir>/model_weights"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory for Grad-CAM outputs. Default: <script_dir>/gradcam_results"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="If set, delete the previous Grad-CAM output folder before running."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Number of classes in the classifier."
    )
...
def get_checkpoint_path(manual_path=None, model_dir=None):
    ...
    candidates.extend(glob.glob(os.path.join(model_dir, "*.pth")))
    candidates.extend(glob.glob(os.path.join(model_dir, "*.txt")))
...
def load_checkpoint_safely(checkpoint_path, device):
    """
    Load a PyTorch state_dict from a checkpoint path.

    This supports checkpoints saved with .pt, .pth, or even .txt
    if the file actually contains a PyTorch state_dict.
    """
    try:
...
def main():
    ...
    print("RUNNING THE FULL REPORT VERSION OF run_gradcam_baseline.py")
    ...
    checkpoint_path = get_checkpoint_path(
        ...
        model_dir=args.model_weights_dir,
    )
